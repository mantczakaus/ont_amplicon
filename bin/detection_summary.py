...
#        blast_df["query_match_seq"] = blast_df[["qseqid2", "qseq"]].apply("\n".join, axis=1)
    ...
    blast_df_final["TARGET_SIZE_FLAG"] = np.where((blast_df_final['sgi'] != 0) & (blast_df_final['query_match_length'] <= float(target_size) + (0.1 * float(target_size))) & (blast_df_final['query_match_length'] >= float(target_size) - (0.1 * float(target_size))), "GREEN", 
                                         np.where(((blast_df_final['sgi'] != 0) & (blast_df_final['query_match_length'] > float(target_size) + (0.1 * float(target_size))) & (blast_df_final['query_match_length'] <= float(target_size) + (0.25 * float(target_size)))) | 
                                                  ((blast_df_final['sgi'] != 0) & (blast_df_final['query_match_length'] < float(target_size) - (0.1 * float(target_size))) & (blast_df_final['query_match_length'] >= float(target_size) - (0.25 * float(target_size)))), "ORANGE",
                                         np.where(((blast_df_final['sgi'] != 0) & (blast_df_final['query_match_length'] < float(target_size) - (0.40 * float(target_size))) | 
                                                  ((blast_df_final['sgi'] != 0) & (blast_df_final['query_match_length'] > float(target_size) + (0.4 * float(target_size)))), "RED",
                                         np.where((blast_df_final['sgi'] == 0), "GREY", "")))))
