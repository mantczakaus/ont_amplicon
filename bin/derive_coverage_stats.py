...
#        blast_df["query_match_seq"] = blast_df[["qseqid2", "qseq"]].apply("\n".join, axis=1)
    ...
    samtools_cov2.rename(columns={"#rname": "qseqid", "endpos": "query_match_length", "numreads": "read_count", "meandepth": "mean_depth"}, inplace=True)
    samtools_cov2['pc_read'] = samtools_cov2['read_count'] / filtered_read_counts * 100
    print(samtools_cov2)
    ...
    print(mosdepth)
    mosdepth['pc_depth_30X'] = np.where(mosdepth['base_counts_at_depth_30X'] > mosdepth['end'], 100, mosdepth['base_counts_at_depth_30X'] / mosdepth['end'] * 100)
    mosdepth['pc_depth_30X'] = mosdepth['pc_depth_30X'].apply(lambda x: float("{:.1f}".format(x)))
    ...
    blast_df_final = blast_df_final.sort_values(["pc_read", "target_organism_match"], ascending = (False, False))
    ...
    blast_df_final['pc_read'] = blast_df_final['pc_read'].apply(lambda x: float("{:.1f}".format(x)))
